...
class ServersTest(unittest.TestCase):
    ...
    @patch("threading.Thread")
    @patch("fakenos.core.servers.TCPServerBase._bind_sockets")
    def test_start_executed_with_address_only(
        self,
        mock_bind_sockets,
        mock_thread,
        mock_thread_event,
        ...
        """
        It passes if the start can be executed correctly
        if only the address is given.
        """
        mock_thread_event().is_set.return_value = False
        
        servers = FakeServer()
        servers.start(address="127.0.0.1")
        mock_bind_sockets.assert_called_once()
        mock_thread_event().set.assert_called_once()
        mock_thread.assert_called_once_with(target=servers._listen)
        mock_thread().start.assert_called_once()
    ...
    @patch("threading.Thread")
    @patch("fakenos.core.servers.TCPServerBase._bind_sockets")
    def test_start_executed_with_port_only(
        self,
        mock_bind_sockets,
        mock_thread,
        mock_thread_event,
    ...
    @patch("threading.Thread")
    @patch("fakenos.core.servers.TCPServerBase._bind_sockets")
    def test_start_executed_with_timeout_only(
        self,
        mock_bind_sockets,
        mock_thread,
        mock_thread_event,
        ...
        """
        It passes if the start can be executed correctly
        if only the timeout is given.
        """
        mock_thread_event().is_set.return_value = False
        
        servers = FakeServer()
        servers.start(timeout=1)
        mock_bind_sockets.assert_called_once()
        mock_thread_event().set.assert_called_once()
        mock_thread.assert_called_once_with(target=servers._listen)
        mock_thread().start.assert_called_once()
    ...
    @patch("threading.Thread")
    @patch("fakenos.core.servers.TCPServerBase._bind_sockets")
    def test_start_executed_with_address_and_port(
        self,
        mock_bind_sockets,
        mock_thread,
        mock_thread_event,
        ...
        """
        It passes if the start can be executed correctly
        if the address and port are given.
        """
        mock_thread_event().is_set.return_value = False
        
        servers = FakeServer()
        servers.start(address="127.0.0.1", port=6000)
        mock_bind_sockets.assert_called_once()
        mock_thread_event().set.assert_called_once()
        mock_thread.assert_called_once_with(target=servers._listen)
        mock_thread().start.assert_called_once()
    ...
    @patch("threading.Thread")
    @patch("fakenos.core.servers.TCPServerBase._bind_sockets")
    def test_start_executed_with_address_and_timeout(
        self,
        mock_bind_sockets,
        mock_thread,
        mock_thread_event,
        ...
        """
        It passes if the start can be executed correctly
        if the address and timeout are given.
        """
        mock_thread_event().is_set.return_value = False
        
        servers = FakeServer()
        servers.start(address="127.0.0.1", timeout=1)
        mock_bind_sockets.assert_called_once()
        mock_thread_event().set.assert_called_once()
        mock_thread.assert_called_once_with(target=servers._listen)
        mock_thread().start.assert_called_once()
    ...
    @patch("threading.Thread")
    @patch("fakenos.core.servers.TCPServerBase._bind_sockets")
    def test_start_executed_with_port_and_timeout(
        self,
        mock_bind_sockets,
        mock_thread,
        mock_thread_event,
        ...
        """
        It passes if the start can be executed correctly
        if the port and timeout are given.
        """
        mock_thread_event().is_set.return_value = False
        
        servers = FakeServer()
        servers.start(port=6000, timeout=1)
        mock_bind_sockets.assert_called_once()
        mock_thread_event().set.assert_called_once()
        mock_thread.assert_called_once_with(target=servers._listen)
        mock_thread().start.assert_called_once()
    ...
    @patch("threading.Thread")
    @patch("fakenos.core.servers.TCPServerBase._bind_sockets")
    def test_start_executed_with_all_arguments(
        self,
        mock_bind_sockets,
        mock_thread,
        mock_thread_event,
        ...
        servers = FakeServer()
        servers.start(address="127.0.0.1", port=6000, timeout=1)
        mock_bind_sockets.assert_called_once()
        mock_thread_event().set.assert_called_once()
        mock_thread.assert_called_once_with(target=servers._listen)
        mock_thread().start.assert_called_once()
