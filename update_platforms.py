...
5. The script will update the tests/core/test_fakenos.py file with the new tests.
"""
...
def check_platforms_in_md(md_file):
    """
    Check which platforms from the given list are also mentioned in the specified markdown file.
    """
...
def download_and_extract_platforms(url: str, local_path: str) -> list[str]:
    ...
    The platforms are extracted from the section under "Supported SSH device_type values".
    """
...
def get_commands(platform_name: str) -> dict[str,]:
    """
...
def get_command_and_output(test_file: str, platform_name: str) -> tuple[str, str]:
    ...
    """
    filename: str = get_files_with_extension_in_folder(f'{tmp_ntc_templates_dir}/tests/{platform_name}/{test_file}', '.raw')[0]
    content: str = ''
...
with open('platforms.md', 'w') as file:
