...
class NosCLI:
    ...
    def __init__(self, nos: Nos) -> None:
        ...
        if self.platform:
            self.env = Environment(
                loader=PackageLoader(f"fakenos.plugins.nos.cli.{self.platform}", "templates"),
                autoescape=select_autoescape(["j2"]),
                trim_blocks=False, lstrip_blocks=False
            )
        self._load_file(os.listdir(f'{os.path.dirname(__file__)}/{self.platform}'))
        
    @classmethod
    def get_nos(cls, nos: Nos) -> "NosCLI":
        """ Abstract factory method to return the correct NOS type."""
    ...
    def _from_module(self, filename: str) -> None:
        ...
        self.device = getattr(module, classname)(nos=self.nos)
    ...
    def _from_file(self, file: str) -> None:
        ...
        """
        filename: str = f'{os.path.dirname(__file__)}/{self.platform}/{file}'
